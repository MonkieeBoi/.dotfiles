...
drawbar(Monitor *m)
{
	...
	x = 0;
	x = drw_text(drw, x, 0, TEXTW(buttonbar) + 4, bh, lrpad / 2, buttonbar, 0);
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
...
quit(const Arg *arg)
{
	...
	if ((fd = fopen(lockfile, "r")) && stat(lockfile, &filestat) == 0) {
		...
		if (filestat.st_ctime <= time(NULL)-1)
			remove(lockfile);
	}
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		wa.background_pixel  = scheme[SchemeAlt][ColBg].pixel;
	...
	/* redraw background */
	XSetForeground(dpy, drw->gc, scheme[SchemeAlt][ColBg].pixel);
